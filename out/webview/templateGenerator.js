...
function generateDashboardHtml(webview, extensionUri, apiKeyConfigured) {
    const nonce = crypto.randomBytes(16).toString('base64');
    const echartsUri = webview.asWebviewUri(vscode_1.Uri.joinPath(extensionUri, 'media', 'echarts.min.js'));
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .interval-btn.active{background:rgba(255,255,255,.08);color:var(--accent)}
...
<body>
  <div class="header">
    ...
    <div class="header-right">
      ...
      <div class="interval-switch" id="interval-switcher">
        <button class="interval-btn" data-sec="10">10s</button>
        <button class="interval-btn" data-sec="30">30s</button>
        <button class="interval-btn" data-sec="60">1m</button>
        <button class="interval-btn active" data-sec="300">5m</button>
        <button class="interval-btn" data-sec="600">10m</button>
        <button class="interval-btn" data-sec="1800">30m</button>
      </div>
    </div>
  </div>
  ...
  <div class="settings-section">
    ...
    <div style="background:var(--accent-dim);border:1px solid rgba(129,140,248,.2);border-radius:var(--radius-md);padding:14px 18px;margin-bottom:12px">
      ...
      <div style="font-size:12px;color:var(--text-secondary);margin-bottom:10px">
        ...
        首次点击按钮会自动安装所需组件（Playwright + Chromium，约 170 MB，仅一次）。
      </div>
  ...
  <script nonce="${nonce}">
      ...
      switch(m.type){
        ...
        case'configUpdate':
          ...
          for(var i=0;i<btns.length;i++){
            var b=btns[i];
            if(parseInt(b.getAttribute('data-sec'))===m.refreshIntervalSeconds)b.classList.add('active');
    ...
    for(var i=0;i<intervalBtns.length;i++){intervalBtns[i].addEventListener('click',function(){
      var sec=parseInt(this.getAttribute('data-sec'));
      intervalBtns.forEach(function(b){b.classList.remove('active')});
      this.classList.add('active');
      vscode.postMessage({type:'setRefreshInterval',seconds:sec});
    })}
