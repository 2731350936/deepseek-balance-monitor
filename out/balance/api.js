...
exports.fetchUserSummary = fetchUserSummary;
exports.fetchPlatformUsage = fetchPlatformUsage;
exports.fetchFullUsage = fetchFullUsage;
...
//  HTTP helper
// ---------------------------------------------------------------
function httpGet(hostname, path, apiKey, timeoutMs, cookie) {
        ...
        const req = https.request({ hostname, path, method: 'GET', headers, timeout: timeoutMs }, (res) => {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode === 200)
                    ...
                    reject({ kind: types_1.ApiErrorKind.ServerError, message: `API 返回 ${res.statusCode}: ${path}` });
            });
        });
        req.on('error', (e) => reject({ kind: types_1.ApiErrorKind.Network, message: `网络错误: ${e.message}` }));
        req.on('timeout', () => { req.destroy(); reject({ kind: types_1.ApiErrorKind.Timeout, message: '请求超时' }); });
        req.end();
    });
}
// ---------------------------------------------------------------
...
function fetchBalance(apiKey, timeoutMs) {
    return httpGet(API_HOST, '/user/balance', apiKey, timeoutMs).then((body) => {
        ...
        if (!(0, types_1.isDeepSeekBalanceResponse)(p))
            throw { kind: types_1.ApiErrorKind.MalformedResponse, message: '余额 API 格式异常' };
        const info = p.balance_infos[0];
        return {
            isoTime: new Date().toISOString(),
            totalBalance: info ? (0, types_1.parseBalanceString)(info.total_balance) : 0,
            grantedBalance: info ? (0, types_1.parseBalanceString)(info.granted_balance) : 0,
            toppedUpBalance: info ? (0, types_1.parseBalanceString)(info.topped_up_balance) : 0,
 ...
 */
async function fetchV1UsageMonth(apiKey, timeoutMs, year, month) {
...
function aggregateV1Records(records) {
    const all = [];
    let monthlyCost = 0;
    for (const r of records) {
        ...
        if (r.cost_in_cents !== undefined) {
            ...
            const pricing = (0, types_1.getModelPricing)(model);
...
//  Platform API calls
// ---------------------------------------------------------------
async function platformGet(path, apiKey, timeoutMs, cookie) {
    try {
        return await httpGet(PLATFORM_HOST, path, apiKey, timeoutMs, cookie);
    }
    catch (e) {
        const err = e;
...
async function fetchUserSummary(apiKey, timeoutMs, cookie) {
...
function usageAmount(usage, types) {
    if (!usage)
        return 0;
...
function flattenModelUsage(mu, date, extraCost) {
    ...
    const pricing = (0, types_1.getModelPricing)(mu.model);
...
async function fetchPlatformUsage(apiKey, timeoutMs, cookie) {
    const now = new Date();
        ...
        try {
            ...
            if (Array.isArray(costArr)) {
                for (const costEntry of costArr) {
                    if (costEntry.days) {
                        for (const day of costEntry.days) {
    ...
    for (const body of [b1, b2]) {
        ...
        try {
                ...
                for (const day of biz.days) {
                    ...
                    if (Array.isArray(subItems)) {
                        ...
                        for (const item of subItems) {
                            ...
                            all.push(flattenModelUsage(item, dayDate, itemCost));
                        }
                    }
                    else if (day.model) {
                        all.push(flattenModelUsage(day, dayDate, dayCost));
                    }
                    else if (day.usage) {
                        all.push(flattenModelUsage(day, dayDate, dayCost));
                    }
            ...
            console.log('[deepseek] parse error: ' + e.message);
        }
    }
...
//  Aggregation
// ---------------------------------------------------------------
function aggregate(items, label) {
...
function buildDailyPoints(items) {
    const map = new Map();
    for (const i of items) {
        const d = i.date || '';
        if (!d)
...
//  Main entry
// ---------------------------------------------------------------
async function fetchFullUsage(apiKey, timeoutMs, cookie) {
    const now = new Date();
    const month = now.getMonth() + 1;
    const year = now.getFullYear();
    const prevMonth = month === 1 ? 12 : month - 1;
    ...
    try {
        ...
        if (pu.platformReachable) {
            for (const item of allItems) {
                const match = pu.dailyItems.find(p => p.date === item.date && p.model === item.model);
                if (match) {
            ...
            if (pu.monthlyCost > monthlyCost)
                monthlyCost = pu.monthlyCost;
        }
        ...
        for (const item of allItems.filter(i => i.date === today)) {
            var m = item.model;
            if (!modelMap[m])
                modelMap[m] = { cost: 0, tokens: 0, cacheHit: 0 };
